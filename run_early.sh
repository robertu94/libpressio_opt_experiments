...
do
for tolerance in $(seq 20 10 100)
do
for inter_iteration in 0 1
do
...
EOF
    ...
    echo "$dataload" "tolerance=$tolerance" "libpressio+sz+early=$inter_iteration"
...
mpiexec pressio -Q  -M all \
       ...
       -o "/pressio/dist_gridsearch/fraz:opt:inter_iteration=$inter_iteration" \
       -o "/pressio/dist_gridsearch:opt:target=5" \
...
EOF
    ...
    echo "$dataload" "tolerance=$tolerance" "libpressio+mgard+early=$inter_iteration"
